...
module.exports = function (script, scriptArgs, nodeArgs, {
  ...
  ignore,
  log_uncaught: logUncaughtExceptions,
  ...
  watcher.on('change', file => {
    ...
    const handleChange = () => {
      ...
      const plural = modifiedFiles.length === 2 ? 'other file' : 'other files';
      const modified = modifiedFiles.length <= 2 ? 'was modified' : 'were modified';
      const message = modifiedFiles[0]
        + (modifiedFiles.length > 1 ? ` and ${modifiedFiles.length - 1} ${plural}` : '')
        + ` ${modified}`;
      notify('Restarting', message);
      modifiedFiles = [];
      watcher.removeAll();
      if (child) {
    ...
    if (debounce > 0) {
      ...
      changeTimeoutId = setTimeout(handleChange, debounce);
    } else {
      handleChange();
    }
  });
  ...
  function start() {
    ...
    child = fork(cmd[0], cmd.slice(1), {
      ...
      env: {
        ...process.env,
        NODE_DEV_OPTS: JSON.stringify({ log_uncaught: logUncaughtExceptions })
      }
    });
